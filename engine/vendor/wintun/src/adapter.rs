...
impl Adapter {
    ...
    pub fn create(
        ...
        let result = unsafe { wintun.WintunCreateAdapter(pool_utf16.as_ptr(), name_utf16.as_ptr(), guid_ptr) };
    ...
    pub fn start_session(self: &Arc<Self>, capacity: u32) -> Result<session::Session, error::WintunError> {
        ...
        if !range.contains(&capacity) {
            return Err(Box::new(error::ApiError::CapacityOutOfRange(error::OutOfRangeData {
                range,
                value: capacity,
            })));
        }
        ...
        if result.is_null() {
            ...
            Ok(session::Session {
                ...
                shutdown_event: unsafe {
                    ...
                    UnsafeHandle(synchapi::CreateEventA(std::ptr::null_mut(), 0, 0, std::ptr::null_mut()))
                },
    ...
    pub fn get_adapter_index(&self) -> Result<u32, error::WintunError> {
        ...
        //stack memory
        let result = unsafe { iphlpapi::GetInterfaceInfo(std::ptr::null_mut(), &mut buf_len as *mut u32) };
        ...
        for interface in interfaces {
            ...
            let open = name
                .chars()
                .position(|c| c == '{')
                .ok_or(format!("Failed to find {{ character inside adapter name: {}", name))?;
            let close = name
                .chars()
                .position(|c| c == '}')
                .ok_or(format!("Failed to find }} character inside adapter name: {}", name))?;
