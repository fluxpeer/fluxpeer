...
fn main() {
    ...
    let wintun =
        unsafe { wintun::load_from_path("examples/wintun/bin/amd64/wintun.dll") }.expect("Failed to load wintun dll");
    ...
    let adapter = match wintun::Adapter::open(&wintun, "Demo") {
        ...
        Err(_) => match wintun::Adapter::create(&wintun, "Example", "Demo", None) {
            Ok(d) => {
                info!("Created adapter successfully! ");
                d
            }
            Err(err) => panic!(
                "Failed to open adapter and failed to create adapter. Is process running as admin? Error: {}",
                err
            ),
        },
    };
        ...
        while RUNNING.load(Ordering::Relaxed) {
            match reader_session.receive_blocking() {
                Ok(mut packet) => {
                    ...
                    writer
                        .write(now.as_secs() as u32, now.subsec_nanos(), bytes, bytes.len() as u32)
                        .unwrap();
                }
                Err(err) => {
