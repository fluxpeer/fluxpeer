...
use crate::anytls_util::{AnyTlsError, Result};
...
pub fn create_server_config_from_files<P: AsRef<Path>>(cert_path: P, key_path: P) -> Result<Arc<ServerConfig>> {
    ...
    let key = rustls_pemfile::private_key(&mut key_reader)
        ...
        .ok_or_else(|| AnyTlsError::Tls(format!("no private key found in {:?}", key_path.as_ref())))?;
...
///
/// When `insecure_skip_verify` is `false` (the default), certificates are validated
/// against the Mozilla root CA bundle via `webpki-roots`.
///
/// When `insecure_skip_verify` is `true`, all certificates are accepted without
/// verification — suitable only for testing with self-signed certificates.
pub fn create_client_config(insecure_skip_verify: bool) -> Result<Arc<ClientConfig>> {
    if insecure_skip_verify {
        tracing::warn!(
            "TLS certificate verification is DISABLED (insecure_skip_verify=true). \
...
mod tests {
    ...
    #[test]
    fn test_create_client_config_secure() {
        let config = create_client_config(false);
        assert!(config.is_ok());
        assert!(Arc::strong_count(&config.unwrap()) >= 1);
    }
