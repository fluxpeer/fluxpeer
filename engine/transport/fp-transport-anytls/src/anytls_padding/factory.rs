use crate::anytls_padding::CHECK_MARK;
use crate::anytls_util::StringMap;
use std::sync::Arc;
...
impl PaddingFactory {
    ...
    pub fn default() -> Arc<Self> {
        DEFAULT_FACTORY
            .get_or_init(|| {
                match Self::new(DEFAULT_PADDING_SCHEME.as_bytes()) {
                    Ok(factory) => Arc::new(factory),
                    Err(e) => {
                        tracing::error!("failed to parse default padding scheme: {e}");
                        // Construct a minimal valid factory as fallback
                        Arc::new(Self {
                            scheme: StringMap::from_bytes(b"stop=0"),
                            raw_scheme: b"stop=0".to_vec(),
                            stop: 0,
                            md5: String::new(),
                        })
                    }
        ...
        for part in parts {
            ...
            if let Some((min_str, max_str)) = part.split_once('-') {
                ...
                if min_val == max_val {
                    ...
                    use rand::Rng;
                    let size = rand::thread_rng().gen_range(min_val..=max_val);
