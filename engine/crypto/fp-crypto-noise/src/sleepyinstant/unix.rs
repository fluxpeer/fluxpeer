...
impl Instant {
    pub(crate) fn now() -> Self {
        // std::time::Instant uses the same pattern; clock_gettime only fails with invalid ClockId
        let t = clock_gettime(CLOCK_ID).expect("known-safe: CLOCK_ID is a valid clock");
        Self { t }
    }
