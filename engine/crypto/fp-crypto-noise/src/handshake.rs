...
use crate::x25519;
use crate::{HandshakeInit, HandshakeResponse};
...
const INITIAL_CHAIN_KEY: [u8; KEY_LEN] = [
    96, 226, 109, 174, 243, 39, 239, 192, 46, 195, 53, 226, 160, 37, 210, 208, 22, 235, 66, 6, 248, 114, 119, 245, 45,
    56, 209, 152, 139, 120, 205, 54,
];

// initiator.chaining_hash = HASH(initiator.chaining_key || IDENTIFIER)
const INITIAL_CHAIN_HASH: [u8; KEY_LEN] = [
    34, 17, 179, 97, 8, 26, 197, 102, 105, 18, 67, 219, 69, 138, 213, 50, 45, 156, 108, 102, 34, 147, 232, 183, 14,
    225, 156, 101, 186, 7, 158, 243,
];
...
pub(crate) fn b2s_hmac(key: &[u8], data1: &[u8]) -> [u8; 32] {
    ...
    let mut hmac = HmacBlake2s::new_from_slice(key).expect("known-safe: HMAC accepts keys of any length");
    hmac.update(data1);
...
pub(crate) fn b2s_keyed_mac_16(key: &[u8], data1: &[u8]) -> [u8; 16] {
    let mut hmac = Blake2sMac::new_from_slice(key).expect("known-safe: Blake2sMac accepts keys 1..=32 bytes");
...
#[inline]
fn aead_chacha20_seal_inner(ciphertext: &mut [u8], key: &[u8], nonce: [u8; 12], data: &[u8], aad: &[u8]) {
    let key = LessSafeKey::new(
        UnboundKey::new(&CHACHA20_POLY1305, key).expect("known-safe: caller provides 32-byte CHACHA20_POLY1305 key"),
    );
    ...
    let tag = key
        ...
        .expect("known-safe: AEAD seal with valid key and nonce cannot fail");
...
fn aead_chacha20_open(
    ...
    data: &[u8],
    aad: &[u8],
) -> Result<(), fp_crypto::Error> {
    let mut nonce: [u8; 12] = [0; 12];
    nonce[4..].copy_from_slice(&counter.to_le_bytes());

    aead_chacha20_open_inner(buffer, key, nonce, data, aad).map_err(|_| fp_crypto::Error::InvalidAeadTag)?;
    Ok(())
}
...
fn aead_chacha20_open_inner(
    ...
    let plaintext = key.open_in_place(Nonce::assume_unique_for_key(nonce), Aad::from(aad), &mut inner_buffer)?;
...
impl TimeStamper {
    ...
    pub(crate) fn new() -> TimeStamper {
        TimeStamper {
            duration_at_start: SystemTime::now()
                .duration_since(SystemTime::UNIX_EPOCH)
                .expect("known-safe: system clock is after UNIX epoch"),
...
impl Tai64N {
    ...
    fn parse(buf: &[u8; 12]) -> Result<Tai64N, fp_crypto::Error> {
        if buf.len() < 12 {
            return Err(fp_crypto::Error::InvalidTai64nTimestamp);
        }
        ...
        let secs = u64::from_be_bytes(
            sec_bytes
                .try_into()
                .expect("known-safe: split_at(8) on 12-byte array yields 8 bytes"),
        );
        let nano = u32::from_be_bytes(
            nano_bytes
                .try_into()
                .expect("known-safe: split_at(8) on 12-byte array yields 4 bytes"),
        );
    ...
    pub(crate) fn after(&self, other: &Tai64N) -> bool {
...
pub(crate) struct NoiseParams {
    ...
    pub(crate) peer_static_public: x25519::PublicKey,
...
#[derive(Debug)]
pub(crate) struct HalfHandshake {
    pub(crate) peer_static_public: [u8; 32],
}
...
pub(crate) fn parse_handshake_anon(
...
) -> Result<HalfHandshake, fp_crypto::Error> {
    ...
    aead_chacha20_open(&mut peer_static_public, &key, 0, packet.encrypted_static, &hash)?;
...
impl Handshake {
    pub(crate) fn new(
        ...
        let params = NoiseParams::new(static_private, static_public, peer_static_public, preshared_key);
    ...
    pub(super) fn receive_handshake_initialization<'a>(
        ...
        packet: HandshakeInit,
        dst: &'a mut [u8],
    ) -> Result<(&'a mut [u8], Session), fp_crypto::Error> {
        // initiator.chaining_key = HASH(CONSTRUCTION)
        let mut chaining_key = INITIAL_CHAIN_KEY;
        // initiator.hash = HASH(HASH(initiator.chaining_key || IDENTIFIER) || responder.static_public)
        let mut hash = INITIAL_CHAIN_HASH;
        ...
        chaining_key = b2s_hmac(&b2s_hmac(&chaining_key, peer_ephemeral_public.as_bytes()), &[0x01]);
        // temp = HMAC(initiator.chaining_key, DH(initiator.ephemeral_private, responder.static_public))
        let ephemeral_shared = self.params.static_private.diffie_hellman(&peer_ephemeral_public);
        let temp = b2s_hmac(&chaining_key, &ephemeral_shared.to_bytes());
        // initiator.chaining_key = HMAC(temp, 0x1)
        chaining_key = b2s_hmac(&temp, &[0x01]);
        // key = HMAC(temp, initiator.chaining_key || 0x2)
        ...
        .map_err(|_| fp_crypto::Error::WrongKey)?;
        ...
        if !timestamp.after(&self.last_handshake_timestamp) {
            // Possibly a replay
            return Err(fp_crypto::Error::WrongTai64nTimestamp);
        }
    ...
    pub(super) fn receive_handshake_response(
    ...
    ) -> Result<Session, fp_crypto::Error> {
        ...
        let ephemeral_shared = state.ephemeral_private.diffie_hellman(&unencrypted_ephemeral);
        let temp = b2s_hmac(&chaining_key, &ephemeral_shared.to_bytes());
        // responder.chaining_key = HMAC(temp, 0x1)
        chaining_key = b2s_hmac(&temp, &[0x01]);
        ...
        let temp = b2s_hmac(&chaining_key, &self.params.preshared_key.unwrap_or([0u8; 32])[..]);
        // responder.chaining_key = HMAC(temp, 0x1)
        chaining_key = b2s_hmac(&temp, &[0x01]);
    ...
    // Compute and append mac1 to a handshake message
    fn append_mac1<'a>(
        &self,
        // local_index: u32,
        dst: &'a mut [u8],
    ...
    pub(super) fn format_handshake_initiation<'a>(
        ...
        // msg.unencrypted_ephemeral = DH_PUBKEY(initiator.ephemeral_private)
        unencrypted_ephemeral.copy_from_slice(x25519::PublicKey::from(&ephemeral_private).as_bytes());
        // initiator.hash = HASH(initiator.hash || msg.unencrypted_ephemeral)
        ...
        let key = b2s_hmac2(&temp, &chaining_key, &[0x02]);
        // msg.encrypted_static = AEAD(key, 0, initiator.static_public, initiator.hash)
        aead_chacha20_seal(encrypted_static, &key, 0, self.params.static_public.as_bytes(), &hash);
        // initiator.hash = HASH(initiator.hash || msg.encrypted_static)
        ...
        self.append_mac1(&mut dst[..super::HANDSHAKE_INIT_SZ])
    }
    ...
    fn format_handshake_response<'a>(
        ...
        let (mut chaining_key, mut hash, peer_ephemeral_public, peer_index) = match state {
            ...
            _ => {
                return Err(fp_crypto::Error::UnexpectedPacket);
            }
        };
        ...
        let dst = self.append_mac1(&mut dst[..super::HANDSHAKE_RESP_SZ])?;
...
mod tests {
    ...
    fn symmetric_chacha20_seal_open() {
        ...
        aead_chacha20_open(&mut [], &key, counter, &encrypted_nothing, &aad).expect("Should open what we just sealed");
    }
}
