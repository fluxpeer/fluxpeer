...
fn chacha20poly1305_ring(key_bytes: &[u8], buf: &mut [u8]) {
    ...
    let tag = key
        .seal_in_place_separate_tag(Nonce::assume_unique_for_key([0u8; 12]), Aad::from(&[]), &mut buf[..n])
        .unwrap();
...
fn chacha20poly1305_non_ring(key_bytes: &[u8], buf: &mut [u8]) {
    ...
    let tag = aead.encrypt_in_place_detached(&nonce, &[], &mut buf[..n]).unwrap();
...
pub fn bench_chacha20poly1305(c: &mut Criterion) {
    ...
    for size in [128, 192, 1400, 8192] {
        ...
        group.bench_with_input(BenchmarkId::new("chacha20poly1305_ring", size), &size, |b, i| {
            let mut key = [0; 32];
        ...
        group.bench_with_input(BenchmarkId::new("chacha20poly1305_non_ring", size), &size, |b, i| {
            let mut key = [0; 32];
            let mut buf = vec![0; i + 16];
