...
impl RawCryptor {
    pub fn new<C: crate::Cryptor>() -> Self {
        let vtable = &VTable {
            init_handshake: |own_prikey: crate::x25519::StaticSecret,
             ...
             -> Result<(std::ptr::NonNull<()>, Vec<u8>), crate::Error> {
                C::init_handshake(own_prikey, node_pubkey).map(|(crypto, hs)| {
                    let ptr = unsafe { std::ptr::NonNull::new_unchecked(Box::into_raw(Box::new(crypto))).cast() };
                    (ptr, hs)
                })
            },
            ...
            handle_handshake_response: |this: *mut (), packet: &[u8]| -> Result<(), crate::Error> {
                C::handle_handshake_response(unsafe { &mut *this.cast::<C>() }, packet)
            },
            on_send: |this: *mut (), packet: &[u8], dst: &mut [u8]| -> Result<&mut [u8], crate::Error> {
                C::on_send(unsafe { &mut *this.cast::<C>() }, packet, dst)
            },
            on_recv: |this: *mut (), packet: &[u8], dst: &mut [u8]| -> Result<&mut [u8], crate::Error> {
                C::on_recv(unsafe { &mut *this.cast::<C>() }, packet, dst)
            },
            get_peer_public: |this: *const ()| -> Result<crate::x25519::PublicKey /* peer pubkey */, crate::Error> {
                C::get_peer_public(unsafe { &*this.cast::<C>() })
            },
            rekey_init: |this: *mut (), dst: &mut [u8]| -> Result<&mut [u8], crate::Error> {
                C::rekey_init(unsafe { &mut *this.cast::<C>() }, dst)
            },
            rekey_respond: |this: *mut (), packet: &[u8], dst: &mut [u8]| -> Result<Option<&mut [u8]>, crate::Error> {
                C::rekey_respond(unsafe { &mut *this.cast::<C>() }, packet, dst)
            },
    ...
    pub fn on_send<'a>(&mut self, packet: &[u8], dst: &'a mut [u8]) -> Result<&'a mut [u8], crate::Error> {
        let this = self.data.ok_or(crate::Error::MustHandshakeFirst)?;
        (self.vtable.on_send)(this.as_ptr(), packet, dst)
    }

    pub fn on_recv<'a>(&mut self, packet: &[u8], dst: &'a mut [u8]) -> Result<&'a mut [u8], crate::Error> {
        let this = self.data.ok_or(crate::Error::MustHandshakeFirst)?;
        (self.vtable.on_recv)(this.as_ptr(), packet, dst)
    }

    pub fn get_peer_public(&self) -> Result<crate::x25519::PublicKey /* peer pubkey */, crate::Error> {
        let this = self.data.ok_or(crate::Error::MustHandshakeFirst)?;
        (self.vtable.get_peer_public)(this.as_ptr())
    }

    /// Rekey from the initiator side on the EXISTING session (gapless): emits a
    /// fresh handshake init while the current session keeps carrying data until
    /// the response installs the new one.
    pub fn rekey_init<'a>(&mut self, dst: &'a mut [u8]) -> Result<&'a mut [u8], crate::Error> {
        let this = self.data.ok_or(crate::Error::MustHandshakeFirst)?;
        (self.vtable.rekey_init)(this.as_ptr(), dst)
    }

    /// Rekey from the responder side on the EXISTING session (gapless): processes
    /// a rekey init, installs the new session alongside the old, returns the resp.
    pub fn rekey_respond<'a>(
        &mut self,
        packet: &[u8],
        dst: &'a mut [u8],
    ) -> Result<Option<&'a mut [u8]>, crate::Error> {
        let this = self.data.ok_or(crate::Error::MustHandshakeFirst)?;
        (self.vtable.rekey_respond)(this.as_ptr(), packet, dst)
    }
}
...
type PacketHandler = for<'a, 'b> fn(*mut (), &'b [u8], &'a mut [u8]) -> Result<&'a mut [u8], crate::Error>;
...
struct VTable {
    ...
    get_peer_public: fn(*const ()) -> Result<crate::x25519::PublicKey /* peer pubkey */, crate::Error>,
    rekey_init: for<'a> fn(*mut (), &'a mut [u8]) -> Result<&'a mut [u8], crate::Error>,
    rekey_respond: for<'a, 'b> fn(*mut (), &'b [u8], &'a mut [u8]) -> Result<Option<&'a mut [u8]>, crate::Error>,
    drop: unsafe fn(*mut ()),
}
...
mod test {
    ...
    impl Cryptor for C1 {
        ...
        fn on_send<'a>(&mut self, _packet: &[u8], dst: &'a mut [u8]) -> Result<&'a mut [u8], crate::Error> {
            println!("c1 on_send");
            dst[0] = 1;
            Ok(dst)
        }

        fn on_recv<'a>(&mut self, _packet: &[u8], dst: &'a mut [u8]) -> Result<&'a mut [u8], crate::Error> {
            println!("c1 on_recv");
            dst[0] = 1;
            Ok(dst)
        }
    ...
    fn raw_crypto() {
        ...
        let handle_handshake = c2.handle_handshake(gen_private(), gen_public(), &[0]).unwrap();
