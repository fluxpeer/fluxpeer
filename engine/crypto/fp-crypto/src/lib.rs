...
// Migrated unsafe-heavy code (FFI/crypto); keep upstream unsafe-fn bodies as-is
...
pub mod x25519 {
    pub use x25519_dalek::{EphemeralSecret, PublicKey, ReusableSecret, SharedSecret, StaticSecret};
}
...
pub fn dst_address(packet: &[u8]) -> Option<std::net::IpAddr> {
    ...
    match packet[0] >> 4 {
        4 if packet.len() >= IPV4_MIN_HEADER_SIZE => {
            let addr_bytes: [u8; IPV4_IP_SZ] = packet[IPV4_DST_IP_OFF..IPV4_DST_IP_OFF + IPV4_IP_SZ]
                .try_into()
        ...
        6 if packet.len() >= IPV6_MIN_HEADER_SIZE => {
            let addr_bytes: [u8; IPV6_IP_SZ] = packet[IPV6_DST_IP_OFF..IPV6_DST_IP_OFF + IPV6_IP_SZ]
                .try_into()
...
pub fn version() -> std::collections::HashMap<String, String> {
    std::collections::HashMap::from([("fp-crypto".to_string(), "1.0.3".to_string())])
}
