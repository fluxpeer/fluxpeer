...
enum PacketProtocol {
    IPv4,
    IPv6,
    /// Unrecognized IP version byte. The value is retained for diagnostic purposes
    /// but not currently read; suppress the warning since this is a protocol enum.
...
impl PacketProtocol {
    ...
    fn to_pi_field(&self) -> Result<u16, io::Error> {
        match self {
            PacketProtocol::IPv4 => Ok(libc::ETH_P_IP as u16),
            PacketProtocol::IPv6 => Ok(libc::ETH_P_IPV6 as u16),
            PacketProtocol::Other(_) => Err(io::Error::other("neither an IPv4 or IPv6 packet")),
        }
    }
    ...
    fn to_pi_field(&self) -> Result<u16, io::Error> {
        Err(io::Error::other("packet information header not implemented on Windows"))
    }
    // MARKER END
}
    ...
    fn encode(&mut self, item: TunPacket, dst: &mut BytesMut) -> Result<(), Self::Error> {
        ...
        match item {
            TunPacket(proto, bytes) if self.0 => {
                ...
                buf.write_u16::<NetworkEndian>(proto.to_pi_field()?)?;
