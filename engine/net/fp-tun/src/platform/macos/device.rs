...
// use libc;
use libc::{AF_INET, SOCK_DGRAM, c_char, c_uint, c_void, sockaddr, socklen_t};
...
impl Device {
    ...
    ///
    /// If `config.raw_fd` is set, the device is created from an existing file descriptor
    /// (e.g., passed from a privileged helper daemon via SCM_RIGHTS). In this case the
    /// TUN socket is already connected and configured — we just read the interface name.
    ///
    /// Otherwise, creates a new utun via PF_SYSTEM kernel control socket (requires root).
    pub fn new(config: &Configuration) -> Result<Self> {
        // --- Path A: use pre-created fd from helper daemon ---
        if let Some(raw_fd) = config.raw_fd {
            let device = unsafe {
                let tun = Fd::new(raw_fd).map_err(|_| io::Error::last_os_error())?;

                // Read interface name from the already-connected utun fd
                ...
                let ctl = Fd::new(libc::socket(AF_INET, SOCK_DGRAM, 0)).map_err(|_| io::Error::last_os_error())?;
            ...
            // Skip configure — the helper daemon already set address/destination/up
            // via ifconfig. Calling configure here would fail with EPERM as non-root.

            return Ok(device);
        }

        // --- Path B: create utun from scratch (requires root) ---
    ...
    /// # Safety
    ///
    /// The caller must ensure `self.name` is a valid interface name that fits in `ifrn.name`.
